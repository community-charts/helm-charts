...
| log.file.location | string | `"/home/node/.n8n/logs/n8n.log"` | Absolute path for the log file. Must be inside one of the writable volumes available in the pod: `/home/node/.n8n` (n8n data — PVC or emptyDir), `/home/node/.cache` (npm cache emptyDir), or `/home/node/.npm` (npm home emptyDir). Paths outside these mounts are read-only when `readOnlyRootFilesystem` is enabled and will cause n8n to fail opening the log file. For a custom path, add a matching `extraVolume` and `extraVolumeMount`. Only for `file` log output. |
